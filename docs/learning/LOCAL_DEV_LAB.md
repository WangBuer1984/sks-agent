...
> **阅读说明（四仓拆分后）**：本文是 2026-07-25~26 的踩坑实录，当时本仓还是 monorepo、服务镜像本地 `build:`。现在三服务走 GHCR 镜像，源码在并列仓。概念仍可用；镜像名 `sks-agent-sks-ai` 之类已过时。生产部署看 `../../deploy/OPS.md`。
>
> 这份是**手把手踩坑实录**，记录从零把本地环境跑通的全过程。每个命令都带预期输出、每个坑都标了根因和修法。
>
> 配套：概念看 `TOOLCHAIN_GUIDE.md`；本地运行速查看 `LOCAL_DEV.md`。
