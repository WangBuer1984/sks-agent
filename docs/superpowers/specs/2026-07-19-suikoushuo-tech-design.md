...
| `recharge_order` | **人工开通订单（商业模式核心）**：用户、套餐（50 条 / 150 条）、金额、手机尾号、状态(**免费体验/已开通**)、站长操作人、开通时间、备注凭证。**用户注册即自动创建订单（状态=免费体验）**；站长核对微信转账后开通，首充把该订单转为已开通、复购新增订单。无「驳回」态——无法核对的转账不入账，站长微信人工询问即可（PRD §11.1）。开通成功即写一笔 `credit_ledger(+N, biz_type=recharge, biz_id=order_id)`；**首充额外写一笔 `+10, biz_type=bonus`（PRD 首充送 10 条体验额度）** |
