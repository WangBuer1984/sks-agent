...
| 项 | 留桩位置 | 联调动作 |
|----|----------|----------|
| 阿里云 SMS 发送（验证码） | `AliyunSmsAuthClient`（DYPNS `SendSmsVerifyCode` 字面码，3 scene：LOGIN_REGISTER/VERIFY_OLD_PHONE/BIND_NEW_PHONE） | 填 `ALIYUN_ACCESS_KEY_ID/SECRET`（空则走 stub 不真发，这是唯一闸门）。签名/端点/三个模板号都不是 env 键，已写死在 `application.yml` 的 `sks.sms.*`——它们不是密钥，且经 .env 有编码与空值覆盖两个坑 |
